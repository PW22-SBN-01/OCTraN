"""
PatchWise training, SOcc DPT
author: Aditya NG (adityang5@gmail.com)

https://github.com/Cardinal-Robo-Taxi/DPT_SOcc
https://github.com/Cardinal-Robo-Taxi/DPT_SOcc/blob/master/patchwise_training.py
"""
import math
...
class PatchWise:
    ...
    The network is trained in a patch-wise manner to reduce the memory requirements.
    Example use
    -----------
    ```
    ...
    def __init__(self, net, train_percentage):
        ...
        self.M = round(self.N * self.train_percentage)
        assert self.M > 0, "The number of parameters to unfreeze is 0, choose a higher training percentage N={}, M={}".format(self.N, self.M)

        self._num_iterations = math.ceil(self.N / self.M)

        # Save all parameter weights
        # This is done so that all the parameters can be updated at once in the end
        self._saved_weights = {}
    ...
    def __next__(self):
        ...
        if self.last_start >= self.N:
            ...
            assert len(self._updated_weights) == self.N, "Not all parameters were updated"
            assert len(self._saved_weights) == self.N, "Not all parameters were saved"
        ...
        #############################################################################
        # Reset all the parameters to the saved weights before starting new iteration
        for index, param in enumerate(self.net.parameters()):
